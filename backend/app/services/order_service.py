...
async def update_order_status(db: AsyncSession, order_id: int, new_status: str) -> Order:
    ...
    if new_status not in allowed:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=f"Cannot transition from '{order.status}' to '{new_status}'")
...
async def convert_order_to_sale(
    ...
    if order.status != "completed":
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Only completed orders can be converted to sales")
    if order.sale_id is not None:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Order already converted to sale")
    if not order.items:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Order has no items to convert")
    ...
    for item in order.items:
        ...
        if product.stock_qty < item.quantity:
            raise HTTPException(
                ...
                detail=f"Insufficient stock for product {item.product_id}: available {product.stock_qty}, requested {item.quantity}",
            )
    ...
    await db.commit()
    sale_result = await db.execute(select(Sale).options(selectinload(Sale.items)).where(Sale.id == sale.id))
